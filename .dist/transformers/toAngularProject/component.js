...
var moduleIndex = exports.moduleIndex = function () {
  ...
  var printFilters = function printFilters(indent, filters) {
    return '' + (filters || []).map(function (_ref4) {
      var name = _ref4.name,
          path = _ref4.path,
          filters = _ref4.filters;
      return indent + '.filter(\'' + camelCase(name) + '\',' + Array(Math.max(1, 33 - name.length - indent.length)).join(' ') + 'require(\'' + path + '\'))' + onNewLineIfExists(printFilters(indent + '  ', filters));
    }).join('\n');
  };
  ...
  return function (_ref5) {
    var name = _ref5.name,
        requirements = _ref5.requirements,
        components = _ref5.components,
        factories = _ref5.factories,
        filters = _ref5.filters,
        routes = _ref5.routes,
        configs = _ref5.configs;
    return autoGenerateWarning('require(\'angular\');\n\n' + (requirements || []).map(function (_ref6) {
      var jsPackageName = _ref6.jsPackageName,
          moduleName = _ref6.moduleName;
      return 'require(\'' + (jsPackageName || '../' + moduleName) + '\');';
    }).join('\n') + '\n\nexport default {\n  \'' + name + '\': angular.module(\'' + name + '\', [' + (requirements || []).map(function (_ref7) {
...
var createApp = exports.createApp = function createApp(_ref8) {
  var rootModule = _ref8.rootModule;
